...
const sheetVariants = cva(
  'fixed z-[61] scale-100 gap-4 bg-dialog-panel p-6 opacity-100 shadow-lg border',
  {
    variants: {
      position: {
...
>(({ position, size, className, sheetClass, showOverlay = true, children, ...props }, ref) => (
  <SheetPortal position={position}>
    ...
    >
      {children}
      <SheetPrimitive.Close className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-dialog-panel transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-secondary">
        <X className="h-4 w-4" />
